...
export async function saveQuizResult(questions, answers, score) {
  ...
  if (wrong.length > 0) {
    ...
    const improvPrompt = `
      Based on these wrong answers:

      ${wrongText}

      Give an improvement tip (max 2 sentences, encouraging).
    `;

    try {
      const r = await model.generateContent(improvPrompt);
      improvementTip = r.response.text().trim();
    } catch {}
  }
